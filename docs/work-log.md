...
2025-11-17 21:47:00 - Enhanced refund system with server-side validation and credit usage API. Implemented comprehensive backend validation in admin.controller.ts to prevent bypassing UI validation (refundReason non-empty, refundAmount > 0, refundAmount <= basePriceUsd). Updated SubscriptionManagementService.cancelWithRefund() to accept custom refundAmount parameter with server-side validation. Created GET /admin/subscriptions/:id/credit-usage endpoint that aggregates token_usage_ledger.credits_deducted for current billing period. Updated frontend SubscriptionManagement.tsx to fetch actual credit usage via refundApi.getSubscriptionCreditUsage() when opening refund modal, removed TODO comment. Fixed incorrect Prisma model reference (usage_history → token_usage_ledger, credits_used → credits_deducted). Backend build successful.
2025-11-17 22:00:00 - Removed all ENTERPRISE_MAX references from frontend codebase. Fixed 17 files: replaced ENTERPRISE_MAX with ENTERPRISE_PRO_PLUS, added missing PRO_PLUS tier to all tier enumerations and mappings. Updated tier display functions in plan109/plan110/plan111.utils.ts to include complete tier set (FREE, PRO, PRO_PLUS, PRO_MAX, ENTERPRISE_PRO, ENTERPRISE_PRO_PLUS, PERPETUAL). Fixed tier filter dropdowns in SubscriptionManagement, UserManagement, and CreditManagement pages. Updated tier color mappings in PlatformAnalytics (added indigo for PRO_PLUS). Removed unused useNavigate imports from AdminHeader and AdminRoute components. Frontend build successful (0 errors, only pre-existing warnings about chunk sizes).
2025-11-17 22:08:00 - Fixed empty subscription detail page issue. Root cause: SubscriptionManagement.tsx had a View button linking to /admin/subscriptions/:id but no route or component existed for this path in adminRoutes.tsx. React Router returned empty page when route not found. Solution: Removed non-functional View button and Link import from SubscriptionManagement.tsx since all subscription details (user, tier, status, price, credits, billing period) are already displayed in the table. Frontend build successful.
[2025-11-17] Fixed verbose tier access logging - Removed debug log from checkTierAccess() to eliminate 18 duplicate logs on model list endpoint. Aggregate logging in ModelService is sufficient. (backend/src/utils/tier-access.ts:88-93)
